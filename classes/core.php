...
class core extends mysqli
{
	...
	function getData($tp = "", $where = "", $w = "*")
	{
		if (is_array($where) && $where) {
			...
			foreach ($where as $key => $value) {
				if ($key == "in") {
					$data .= " id IN ('" . str_replace(",", "','", $value) . "')" . ((count($where) - 1) > $i ? " and " : " ");
				} else
					$data .=  "`" . $key . "`=" . $value . ((count($where) - 1) > $i ? " and " : " ");
				$i++;
			}
			$where = $data;
		}
	...
	function getcertificates($profpram)
	{
		...
		if (array_key_exists('special', $profpram)) {
			$profpram["special"] = (int)$profpram["special"];
			$condition .= "and special = $profpram[special]";
		}
	...
	function get_sections()
	{
		...
		while ($columns = $result->fetch_array(MYSQLI_ASSOC)) {
			$sec["id"] = $columns["id"];
			$sec["title"] = $columns["title"];
			$sec["title_arabic"] = $columns["title_arabic"];
			$sec["icon"] = $columns["icon"];
			$sec["english_txt"] = $columns["english_txt"];
			$sec["arabic_txt"] = $columns["arabic_txt"];
			$sec["active"] = $columns["active"];
	...
	//***************************************************************************************
	//***************************************************************************************
	function getcertificates2($parm)
	{
